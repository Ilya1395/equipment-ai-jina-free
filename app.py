...
[data-testid="stDataFrame"] { background: #f7fbff !important; }
a { color: #083d77 !important; font-weight: 700; }
.small-title { font-size: 1.75rem; line-height: 1.2; font-weight: 800; margin-bottom: .25rem; }
.subtitle { font-size: 1.0rem; margin-bottom: 1rem; }
.source-card { background:#f7fbff; border:1px solid #9ec5e8; border-radius:10px; padding:10px 12px; margin:6px 0; }
</style>
"""
st.markdown(CUSTOM_CSS, unsafe_allow_html=True)

st.markdown('<div class="small-title">Поиск характеристик моделей оборудования</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">Бесплатная схема: Jina Search/Reader для поиска и чтения источников + Hugging Face для ИИ-извлечения характеристик.</div>', unsafe_allow_html=True)
...
class_name = st.text_input("Класс", value="Насосы")
subclass_name = st.text_input("Подкласс", value="Погружные")
model_code = st.text_input("Код модели", value="ГНОМ 40-25")
...
if run:
    ...
    for src in valid_sources[:max_sources]:
        st.markdown(
            f'<div class="source-card"><b>{html.escape(src.title)}</b><br>{source_link(src.url or src.title, src.url)}<br><small>{html.escape(src.snippet[:300])}</small></div>',
    ...
    df = df.drop_duplicates(subset=["Характеристика", "Значение", "Ед. изм.", "Источник для экспорта"])
    ...
    st.info("Введите данные модели и нажмите «Найти характеристики». Для полностью бесплатного запуска нужен только бесплатный Hugging Face token; Jina Search может работать без ключа с ограничениями.")
